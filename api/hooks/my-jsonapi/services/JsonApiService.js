...
module.exports = {
  ...
  CONTEXT_ANY: jsonApiValidator.CONTEXT_ANY,
  CONTEXT_UPDATE: jsonApiValidator.CONTEXT_UPDATE,
  CONTEXT_CREATE: jsonApiValidator.CONTEXT_CREATE,

  init: function(config) {
    sails.log.verbose("my-jsonapi: running service init");
    this.config = config;
    for (let name in sails.models) {
      ...
      model.associations.forEach((association) => {
        ...
        associations[alias] = {
          type: pluralize(type),
          links: function(data, extraData) {
            return {
              'self':`${config.prefix}/${identity}/${data.id}/relationships/${alias}`,
              'related': `${config.prefix}/${identity}/${data.id}/${alias}`
            };
          }
        };
      });
  ...
  getAttributesSerializedCaseSetting: function() {
    return this.config.caseSerialized;
  ...
  getAttributesDeserializedCaseSetting: function() {
    return this.config.caseDeserialized;
    // var caseSetting = undefined;
    //
    // if (sails.config.jsonapi !== undefined) {
  ...
  deserialize: function(modelName, data) {
    ...
    returnedValue = Serializer.deserialize(modelName, caseSetting !== undefined ? this._convertCase(data, caseSetting) : data);
  ...
  serializeRelationship: function(serializedRelationshipData, options) {
    let relData;
    if (options.type === 'model') {
      relData = serializedRelationshipData.data ?
        { type: serializedRelationshipData.data.type, id: serializedRelationshipData.data.id } :
        null;
    } else {
      relData = serializedRelationshipData.data.map(it => {return {type: it.type, id: it.id}; });
    }
  ...
  deserializeRelationship: function(data, options) {
    return options.type === 'model' ? (data.data ? data.data.id : null) : (data.data || []).map(it => it.id);
  },
